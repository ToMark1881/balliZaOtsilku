...
extension String {
    ...
    var html2AttributedString: NSAttributedString? {
        ...
        do {
            ...
            for item in array {
                if !item.isEmpty {
                    let range = NSMakeRange(0, item.count)
            ...
            attrStrModified.enumerateAttribute(NSAttributedString.Key.link , in: NSMakeRange(0, attrStrModified.length), options: [.longestEffectiveRangeNotRequired]) { value, range, isStop in
                if let value = value {
